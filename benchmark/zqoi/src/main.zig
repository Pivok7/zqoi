...
fn testDecode(
    ...
    for (0..options.runs + options.warmup) |i| {
        //START
        const start_time = std.Io.Timestamp.now(io, .real).toNanoseconds();

        const image = try zqoi.Image.fromBuffer(allocator, pixels);

        const end_time = std.Io.Timestamp.now(io, .real).toNanoseconds();
        //END
...
fn benchFile(
    ...
    const res = try testEncode(allocator, io, zimage, options);
    defer allocator.free(res.pixels);

    const decode_time = try testDecode(allocator, io, res.pixels, options);
...
fn benchmarkDir(
    ...
    var dir = cwd().openDir(io, path, .{ .iterate = true }) catch {
        const time = try benchFile(allocator, io, path, options);
    ...
    while (try dir_iterator.next(io)) |d| : (total_images += 1) {
        const name = d.name;

        const new_path = try std.fmt.bufPrintZ(&buf, "{s}/{s}", .{ path, name });

        const res = try benchmarkDir(allocator, io, new_path, options);
...
pub fn main(init: std.process.Init) !void {
    ...
    var dir = cwd().openDir(io, bench_dir_path, .{}) catch |err| {
    ...
    _ = try benchmarkDir(allocator, io, bench_dir_path, options);
}
